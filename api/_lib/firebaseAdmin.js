...
async function verifyMasterToken(req) {
  ...
  if (!callerEmail) return { ok: false, status: 403 };
...
/**
 * Verifica token Firebase Auth (qualquer usuário autenticado, não só master)
 */
...
async function setPanelUser(uid, data) {
  ...
  try {
    ...
    console.error('[setPanelUser] Erro:', uid, err);
    return false;
  }
}
...
async function removePanelUser(uid) {
  ...
  try {
    ...
    console.error('[removePanelUser] Erro:', uid, err);
    return false;
  }
}
...
async function listPanelUsersFromDb() {
  ...
  try {
    ...
    console.error('[listPanelUsersFromDb] Erro:', err);
    return [];
  }
}
...
async function getLicense(key) {
  ...
  try {
    ...
    console.error('[getLicense] Erro:', key, err);
    return null;
  }
}
...
async function updateLicense(key, updates) {
  ...
  try {
    ...
    console.error('[updateLicense] Erro:', key, err?.message || err);
    return false;
  }
}
